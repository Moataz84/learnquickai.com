...
app.prepare().then(() => {
  ...
  io.on("connection", socket => {
    ...
    socket.on("user-id", userId => socket.userId = userId)

    socket.on("init-game", () => {
      const gameId = randomBytes(8).toString("hex")
      socket.join(gameId)
      writeGameData(gameId, {gameId, started: false, createdBy: socket.userId, users: [{id: socket.userId}]})
    ...
    socket.on("player-join", gameId => {
      ...
      if (readGameData(gameId).started) socket.emit("game-started")
    })
    ...
    socket.on("answer-selected", data => {
      if (readGameData(data.gameId)?.users?.find(user => user.id === data.user.id)) {
        writeGameData(data.gameId, {users: [...readGameData(data.gameId).users.filter(user => user.id !== data.user.id), data.user]})
      } else {
        writeGameData(data.gameId, {users: [...readGameData(data.gameId).users, data.user]})
      }
    })
